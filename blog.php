...
class TinamboBlog {
    ...
    public function loadExternalFile($file, $type = 'template', $data = null) {
        $file = strip_tags(str_replace('..', '', trim($file)));
        switch ($type) {
            case 'css':
                ...
                for ($i = 0; $i < count($pages); $i++) {
                    if ($pages[$i]->getParent() === false) {
                        $out .= '<tr>
                            ...
                            <td>' . ($pages[$i]->isPublished() ? '<span class="entypo-check"></span>' : '') . '</td>
                            <td>
                ...
                if (count($posts) > 0) {
                    for ($i = 0; $i < count($posts); $i++) {
                        $out .= '<tr>
                            ...
                            <td>' . ($posts[$i]->isSticky() ? '<span class="entypo-check"></span>' : '') . '</td>
                            <td>' . ($posts[$i]->isPublished() ? '<span class="entypo-check"></span>' : '') . '</td>
                            <td>
...
class TinamboPost {
    ...
    public function save() {
        return file_put_contents($this->getFile(), $this->getJSON());
    }
    ...
    public function saveAs($filename) {
        return file_put_contents($filename, $this->getJSON());
    }
    ...
    public function get($file) {
        ...
        switch ($file) {
            case 'main':
                ...
                $out .= "/* Tinambo v" . $this->blog->getVersion() . " https://vox.space */$(document).ready(function(){ $('#navigation-small').change(function(){window.location = $(this).find('option:selected').val();});" .
                    ($this->blog->config->get('picozuKey') ? "$('[data-popup]').ClassyPicozu({key:'" . $this->blog->config->get('picozuKey') . "'});" : "") .
                    "});\n";
                break;
            case 'ga':
